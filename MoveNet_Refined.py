...
while not end: # Repeat indefinitely as long not "q"
    ...
    while fails < 3 and not end and not cut and not restart:
        ...
        #game loop
        while finish - start < 5 and not end and not skipEvent and not cut and not restart:
            collision = False
            ret, frame = cap.read()
    ...
    while not end and not restart:
        frame = cv2.rectangle(frame, (0, 0), (videoWidth, videoHeight), (255, 255, 255), thickness=-1)
        frame = cv2.putText(frame, "Rounds Survived: " + str(rounds), (int(videoWidth / 2) - 300, int(videoHeight / 2) - 70), \
                            fontface, \
                            fontscale, fontcolor, thickness=3)
        frame = cv2.putText(frame, "Score: " + str(score), (int(videoWidth / 2) - 150, int(videoHeight / 2) - 25), fontface, \
                            fontscale, fontcolor, thickness=3)
        frame = cv2.putText(frame, "Press q to quit", (int(videoWidth / 2) - 250, int(videoHeight / 2) + 25), fontface, \
                            fontscale, fontcolor, thickness=3)
        frame = cv2.putText(frame, "Press r to restart", (int(videoWidth / 2) - 300, int(videoHeight / 2) + 75), fontface, \
                            fontscale, fontcolor, thickness=3)
        cv2.imshow(name, frame)
        input = cv2.waitKey(10)
        if input == ord('c') or cv2.waitKey(10) & 0xFF == ord('q'):
            end = True
        if input == ord('r'):
            restart = True
        if cv2.getWindowProperty(name, cv2.WND_PROP_VISIBLE) < 1:
